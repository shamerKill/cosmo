...
class _HttpToolApp extends UriTool {
  ...
  Future<TokenModel?> getBaseCoin() {
    return HttpToolClient.getHttp(customUri('token/params'))
      .then(
        (res) {
  ...
  Future<HttpToolResponse?> getAccountSenderList(String address, { page = 1, limit = 10 }) {
    return HttpToolClient.getHttp(
      customUri('/cosmos/tx/v1beta1/txs', query: "events=message.module='bank'&events=transfer.sender='$address'&pagination.limit=$limit&pagination.offset=${(page - 1) * limit}&order_by=ORDER_BY_DESC")
    );
  }
  // 获取账户接收记录
  Future<HttpToolResponse?> getAccountRecipientList(String address, { page = 1, limit = 10 }) {
    return HttpToolClient.getHttp(
      customUri('/cosmos/tx/v1beta1/txs', query: "events=message.module='bank'&events=transfer.recipient='$address'&pagination.limit=$limit&pagination.offset=${(page - 1) * limit}&order_by=ORDER_BY_DESC")
    );
  }
  // 获取账户发送/接收记录次数
  ...
  Future<HttpToolResponse> getChainTokensList(int page, { limit = 10 }) {
    return HttpToolClient.getHttp(
      customUri('/token/tokens', queryParameters: { 'pagination.limit': '$limit', 'pagination.offset': ((page - 1) * limit).toString(), 'pagination.count_total': 'true' })
  ...
  Future<HttpToolResponse?> getAccountDelegateData(String address) {
    return HttpToolClient.getHttp(customUri('/cosmos/staking/v1beta1/delegations/$address', queryParameters: { 'pagination.limit': '100000'}))
      .then(
        (res) => res,
        onError: (e) => null,
      );
  ...
  Future<HttpToolResponse?> getAccountUnDelegateData(String address) {
    return HttpToolClient.getHttp(customUri('/cosmos/staking/v1beta1/delegators/$address/unbonding_delegations', queryParameters: { 'pagination.limit': '100000'}))
      .then(
        (res) => res,
        onError: (e) => null,
      );
  ...
  Future<HttpToolResponse?> getAccountRewardData(String address, { String? vAddress }) {
    return HttpToolClient.getHttp(customUri('/cosmos/distribution/v1beta1/delegators/$address/rewards' + (vAddress == null ? '' : '/$vAddress'), queryParameters: { 'pagination.limit': '100000'}))
      .then(
        (res) => res,
        onError: (e) => null,
      );
  ...
  Future<HttpToolResponse?> getChainVerifierInfo(String vAddress) {
    return HttpToolClient.getHttp(customUri('/cosmos/staking/v1beta1/validators/$vAddress'))
      .then(
        (res) => res,
        onError: (e) => null,
      );
  ...
  Future<HttpToolResponse?> getAccountReDelegateData(String address) {
    return HttpToolClient.getHttp(customUri('/cosmos/staking/v1beta1/delegators/$address/redelegations'))
      .then(
        (res) => res,
        onError: (e) => null,
      );
  ...
  Future<HttpToolResponse> getChainVerifiersList(int page, { limit = 10 }) {
    return HttpToolClient.getHttp(
      customUri('/cosmos/staking/v1beta1/validators', queryParameters: { 'pagination.limit': '$limit', 'pagination.offset': ((page - 1) * limit).toString(), 'pagination.count_total': 'true' })
    ).then((res) => res..data = res.data['validators']);
  }
  // 获取所有提案列表
  Future<HttpToolResponse> getChainProposalsList(int page, { limit = 10 }) {
    return HttpToolClient.getHttp(
      customUri('/cosmos/gov/v1beta1/proposals', queryParameters: { 'pagination.limit': '$limit', 'pagination.offset': ((page - 1) * limit).toString(), 'pagination.count_total': 'true' })
  ...
  Future<HttpToolResponse> getChainProposalDetailProposer(String id) {
    return HttpToolClient.getHttp(
      customUri('/gov/proposals/$id/proposer')
    ).then((res) => res);
  }
  // 提案存款人
  ...
  Future<HttpToolResponse> getChainCreateTokenFee(String symbol) {
    return HttpToolClient.getHttp(
      customUri('/token/fee/$symbol')
    ).then((res) => res);
  }
  // 获取交易详情
  Future<HttpToolResponse> getChainLogDetail(String hash) {
    return HttpToolClient.getHttp(
      customUri('/cosmos/tx/v1beta1/txs/$hash')
    ).then((res) => res);
  }
}
...
class _HttpToolServer extends UriTool {
  ...
  Future<HttpToolResponse> getChainFee() {
    return HttpToolClient.getHttp(
      customUri('rpc/tx_fee')
    );
  }
  /// 获取公告列表
  Future<HttpToolResponse> getNotificationList(int page, { limit = 10 }) {
    return HttpToolClient.getHttp(
      customUri('news/list', queryParameters: { 'page': page.toString(), 'limit': limit.toString() })
    );
  }
  /// 获取公告详情
  ...
  Future<HttpToolResponse> postProblems(FormData data) {
    return HttpToolClient.postHttp(
      customUri('rpc/desk/submit'), data: data
    );
  }
  /// dapp列表分类
  Future<HttpToolResponse> getDappTypes() {
    return HttpToolClient.getHttp(customUri('rpc/dapp/type', queryParameters: { 'page': '1', 'limit': '10000'}));
  }
  /// dapp列表
  Future<HttpToolResponse> getDappList(String type) {
    return HttpToolClient.getHttp(customUri('rpc/dapp', queryParameters: { 'type': type, 'page': '1', 'limit': '10000'}));
  }
  /// dapp广告图
  ...
  Future<HttpToolResponse?> getChainVerifierAvatar(List<String> vAddressList) {
    return HttpToolClient.getHttp(customUri('rpc/validators/avatar', query: vAddressList.map((_item) => 'address=$_item').join('&')))
      .then(
        (res) => res,
        onError: (e) => null,
      );
  ...
  Future<HttpToolResponse> searchTokenInfo(String tokenHand) {
    ...
    // 判断hex地址
    if (tokenHand.startsWith('0x') && tokenHand.length == 42) return searchToken20Info(tokenHand);
    // 判断bech32地址
    if (tokenHand.startsWith(Env.envConfig.chainInfo.addressPrefix) && tokenHand.length == 41) {
...
// 格式化代币数据
TokenModel _resFormatToToken(dynamic _token, { enumTokenType? type, String? contractAddress }) {
  return TokenModel()
