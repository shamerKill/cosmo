...
class AccountModel extends _StorageBaseAbstract {
  ...
  set tokenList(List<TokenModel> value) {
    _tokenList = value;
    _saveKey();
  }
  ...
  set verifierList(List<UserVerifierModel> value) {
    _verifierList = value;
    _saveKey();
  }
  ...
  /// 账户类型， 普通账户/观察账户
  EnumAccountClass get accountClass => _accountClass;
  set accountClass(EnumAccountClass value) {
    _accountClass = value;
    _saveKey();
  }

  EnumAccountClass _accountClass = EnumAccountClass.normal;

  /// 账户储存的dapp权限列表
  List<DappModel> get dappPermissionList => _dappPermissionList;
  set dappPermissionList(List<DappModel> value) {
    _dappPermissionList = value;
    _saveKey();
  }
  ...
  void _saveKey() {
    ...
    _valueMap['accountClass'] = accountClass;
  }
  ...
  void setData(String sourceStr) {
    ...
    createTime = source['createTime'] != null
        ? DateTime.tryParse(source['createTime'])
        : null;
    accountType = StringTool.accountStringToType(source['accountType']);
    tokenList = (source['tokenList'] ?? [])
        .map<TokenModel>((dynamic item) => TokenModel()..setData(item))
        .toList();
    verifierList = (source['verifierList'] ?? [])
        .map<UserVerifierModel>(
            (dynamic item) => UserVerifierModel()..setData(item))
        .toList();
