...
class _AppTheme {
  ...
  ThemeData get theme {
    if (_type == EnumThemeType.light) {
      return Env.envConfig.isRelease
          ? _lightTheme
          : _getTheme(AppColor.lightColors, sizes);
    } else if (_type == EnumThemeType.dart) {
      return Env.envConfig.isRelease
          ? _darkTheme
          : _getTheme(AppColor.darkColors, sizes);
    }
    return Env.envConfig.isRelease
        ? _lightTheme
        : _getTheme(AppColor.lightColors, sizes);
...
ThemeData _getTheme(ThemeColorClass colors, AppSize sizes) => ThemeData(
      ...
      elevatedButtonTheme: ElevatedButtonThemeData(
        style: ButtonStyle(
          ...
          textStyle:
              MaterialStateProperty.all(TextStyle(color: colors.hightColor)),
      ...
      inputDecorationTheme: InputDecorationTheme(
        ...
        floatingLabelStyle:
            TextStyle(color: colors.primaryColor.withOpacity(0.5)),
        hintStyle: TextStyle(
            color: colors.textGray.withOpacity(0.5),
            fontSize: sizes.inputFontSize),
        prefixIconColor: MaterialStateColor.resolveWith((states) =>
            states.any({MaterialState.focused}.contains)
                ? colors.primaryColor
                : colors.textGray),
        ...
        enabledBorder: OutlineInputBorder(
          borderSide: BorderSide(color: colors.textGray.withOpacity(0.3)),
          borderRadius: BorderRadius.circular(sizes.radius),
        ),
        ...
        contentPadding: EdgeInsets.symmetric(
            vertical: sizes.basic * 10, horizontal: sizes.basic * 20),
        filled: true,
      ),
