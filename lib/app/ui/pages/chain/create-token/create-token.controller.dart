...
class ChainCreateTokenPageController extends GetxController {
  ...
  TextEditingController coinScaleVolumeTextController =
      TextEditingController(text: scaleList[0]);
  TextEditingController coinMintableVolumeTextController =
      TextEditingController(text: mintableList[0]);
  ...
  _getTokenCreateFee() async {
    ...
    state.createFee = '';
    var result =
        await httpToolApp.getChainCreateTokenFee(coinSymbolTextController.text);
    LLoading.dismiss();
    if (result.status == 0 && result.data != null) {
      ...
      state.createFee =
          NumberTool.amountToBalance(result.data['issue_fee']['amount']);
    }
  }
  ...
  onExchangeScale() async {
    ...
    LBottomSheet.baseBottomSheet(
      ...
      child: LBottomSheet.selectSheetChild(
          selected: scaleList
              .indexWhere((e) => e == coinScaleVolumeTextController.text),
          labelList: scaleList,
          successCallBack: (int type) {
            ...
            if (coinTotalVolumeTextController.text != '') {
              ...
              if (scale > oldScale) {
                total =
                    total * BigInt.from(pow(10, (scale - oldScale).toInt()));
              } else if (scale < oldScale) {
                total =
                    total ~/ BigInt.from(pow(10, (oldScale - scale).toInt()));
              }
              coinTotalVolumeTextController.text =
                  NumberTool.getNumberInt(total.toString());
            }
            Get.back();
          }),
    );
  ...
  onExchangeMintable() {
    ...
    LBottomSheet.baseBottomSheet(
      showClose: false,
      child: LBottomSheet.selectSheetChild(
          selected: mintableList
              .indexWhere((e) => e == coinMintableVolumeTextController.text),
  ...
  createToken() async {
    ...
    if (coinNameTextController.text == '' ||
        ...
        coinTotalVolumeTextController.text == '' ||
        state.createFee == '' ||
        state.isLoading) return LToast.warning('ErrorWithInputData'.tr);
    ...
    dataAccount.updateAccount(dataAccount.state.nowAccount!
