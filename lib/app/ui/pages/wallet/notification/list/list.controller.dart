...
class WalletNotificationListPageState {
  ...
  // 列表消息数据
  final RxList<NotificationModel> listNotification =
...
class WalletNotificationListPageController extends GetxController {
  ...
  onGetNoticeList() async {
    ...
    var result =
        await httpToolServer.getNotificationList(state.page, limit: 10);
    LLoading.dismiss();
    if (result.status != 0) return;
    if (result.data == null) return state.page = 0;
    if (result.data.length < 10) state.page = 0;
    for (var _item in result.data) {
      state.listNotification.add(NotificationModel()
        ..id = '${_item['id']}'
        ..title = _item['title']
        ..time = _item['create_time']
        ..read = dataNotificationId.state.notificationIdList
            .contains('${_item['id']}'));
    }
