...
class WalletTokenLogsDetailPageController extends GetxController {
  ...
  _setTitle(TransferLogModel item) {
    ...
    } else if (_formatData is Map && _formatData['type'] == enumTokenType.prc20) {
    ...
    if (_formatData is ModelMsgSend) { // 单对单转账
      if (_formatData.fromAddress == accountAddress) { // 转出
        for (var _item in _formatData.amount) {
          ...
          _tokenRes.amount = '-' + NumberTool.amountToBalance(_item.amount, scale: _tokenRes.scale);
          state.amountVolume.add(_tokenRes);
        }
      }
      if (_formatData.toAddress == accountAddress) { // 转入
        for (var _item in _formatData.amount) {
          var _tokenRes = await dataToolServer.getTokenInfo(_item.denom);
          _tokenRes.amount = '+' + NumberTool.amountToBalance(_item.amount, scale: _tokenRes.scale);
          state.amountVolume.add(_tokenRes);
      ...
      for (var _item in _formatData.inputs) {
        if (_item.address == accountAddress) {
          for (var _item2 in _item.coins) {
            ...
            _tokenRes.amount = '-' + NumberTool.amountToBalance(_item2.amount, scale: _tokenRes.scale);
            state.amountVolume.add(_tokenRes);
          }
        }
      }
      ...
      if (_formatData.delegatorAddress == accountAddress) {
        var _tokenRes = await dataToolServer.getTokenInfo(_formatData.value.denom);
        _tokenRes.amount = '-' + NumberTool.amountToBalance(_formatData.value.amount, scale: _tokenRes.scale);
        state.amountVolume.add(_tokenRes);
      }
    } else if (_formatData is ModelMsgEditValidator) { // 编辑验证者
    } else if (_formatData is ModelMsgDelegate) { // 验证者委托
      if (_formatData.delegatorAddress == accountAddress) {
        var _tokenRes = await dataToolServer.getTokenInfo(_formatData.amount.denom);
        _tokenRes.amount = '-' + NumberTool.amountToBalance(_formatData.amount.amount, scale: _tokenRes.scale);
        state.amountVolume.add(_tokenRes);
      }
    } else if (_formatData is ModelMsgBeginRedelegate) { // 验证者转质押
    } else if (_formatData is ModelMsgUndelegate) { // 验证者取消质押
      ...
      if (_formatData.withdrawerAddress == accountAddress) {
        var _tokenRes = await dataToolServer.getTokenInfo(_formatData.poolCoin.denom);
        _tokenRes.amount = '+' + NumberTool.amountToBalance(_formatData.poolCoin.amount, scale: _tokenRes.scale);
        state.amountVolume.add(_tokenRes);
      }
    } else if (_formatData is ModelMsgSwapWithinBatch) { // liquidity交易
    }
  ...
  _showPrc20Volume(TransferLogModel item) async {
    LLoading.showLoading();
    String direction = item.rawLog['from'] == dataAccount.state.nowAccount?.address ? '- ' : ' ';
    state.amountVolume.add(
      TokenModel()
    ...
    state.logDetail.fee
      ...
      ..amount = ((BigInt.parse(result?.data['data']['GasLimit']) * BigInt.parse(result?.data['data']['GasPrice'])) ~/ BigInt.from(1000)).toString();
  ...
  // 查看详情
  goToDetail() {
    Get.toNamed(PlugRoutesNames.dappWebview, parameters: { 'link': base64.encode(utf8.encode(
