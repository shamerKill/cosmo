...
class WalletTokenLogsPageController extends GetxController
  ...
  _initTokenInfo(String tokenMark) async {
    ...
    var resultAmount = await httpToolApp.getAccountBalance(
        state.accountInfo.address, tokenMark);
  ...
  Future<void> _getTransferLogsPRC10(bool refresh) async {
    ...
    HttpToolResponse? result = await httpToolServer.getAccountLogsList(
        dataAccount.state.nowAccount?.address ?? '', state.tokenInfo.minUnit,
        page: state.logsPageAll, limit: 10);
  ...
  Future<void> _getTransferLogsPRC20(bool refresh) async {
    ...
    if (result == null) return;
    if (result.data == null ||
        result.data['errno'] != 200 ||
        result.data['data'] is! Map ||
        result.data['data']['List'] is! Iterable) {
      state.logsPageAll = 0;
      return;
